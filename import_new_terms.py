...
def import_new_terms():
    ...
    # Get only the new terms (rows 85-115)
    new_terms_df = df.iloc[84:115]  # Python uses 0-based indexing
    ...
    with app.app_context():
        ...
        category_keywords = {
            'Body Parts': ['organ', 'tissue', 'body', 'bone', 'muscle', 'blood', 'anatomy'],
            'Diseases': ['disease', 'infection', 'disorder', 'syndrome', 'condition', 'virus'],
            'Symptoms': ['pain', 'ache', 'feeling', 'discomfort', 'symptom', 'sign'],
            'Medications': ['medicine', 'drug', 'pill', 'medication', 'tablet', 'treatment'],
            'Medical Procedures': ['surgery', 'procedure', 'test', 'scan', 'examination'],
            'Medical Equipment': ['device', 'tool', 'machine', 'equipment', 'instrument'],
            'Emergency Care': ['emergency', 'urgent', 'critical', 'immediate'],
            'Mental Health': ['mental', 'psychological', 'emotional', 'behavioral'],
            'Reproductive Health': ['pregnancy', 'birth', 'reproductive', 'fertility'],
            'Diagnostic Terms': ['diagnosis', 'assessment', 'evaluation']
        }
        ...
        for idx, row in new_terms_df.iterrows():
            term_en = row['English Term'].strip()
            definition_en = row['English Definition'].strip()
